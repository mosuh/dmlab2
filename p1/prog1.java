...
rc[i].attr3=Integer.parseInt(dataArray[3]);
if(rc[i].attr1 > max)
max=rc[i].attr1; 
if(rc[i].attr1 < min)
min=rc[i].attr1;
System.out.println(rc[i].index+" "+rc[i].attr1+" "+rc[i].attr2+" "+rc[i].attr3);
                  data=CSV.readLine();
i++;
 }
...
avg += rc[j].attr1; 
avg = avg/i;
System.out.println("max value :"+max+"\tmin value:"+min); System.out.println("Average value is: "+avg);
